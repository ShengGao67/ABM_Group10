...
to setup-turtles
  ...
  create-cops initial-cop-density * (initial-defect-cop-percent / 100) * count patches
  [
    move-to one-of patches with [not any? turtles-here]
    set color black
    set size 0.9
    set hardship 0.5 + random-float 0.5
    set not-defected? true
  ]
end
...
to cop-rule ;; The cop rule
  ask cops [
    ...
    set grievance hardship * (1 - legitimacy) ;; G = H (1 - L) ;; the same formulation as used for rebels
    ifelse grievance > defect-t
    [become-defect]
    [become-non-defect]

...
to setup
  ...
  if initial-cop-density + initial-agent-density > 100 [
   ...
   if M1 + M2 + M3 + M4 > 100 [
    user-message (word
      "The Ratio of Temperaments should not exceed 100 "
      "should not be greater than 100.")
    stop
  ]

  ; create cops
  set ratio1 initial-cop-density  * 0.35
  set ratio2 (initial-cop-density - ratio1) * 0.46
  set ratio3 (initial-cop-density - ratio2 - ratio1)
  ;set Melancholic_Density (initial-cop-density * 0.35)
  ;set Phlegmatic_Density (initial-cop-density * 0.30)
  ;set Sanguine_Density (initial-cop-density * 0.20)
  ;set Choleric_Density (initial-cop-density * 0.15 * .01 * count patches)

  create-cops round (initial-cop-density * .01 * count patches) [
    move-to one-of patches with [ not any? turtles-here ]
    set v 0
    set Defected? false
    set NotDefected? true
    display-cop1
  ]

  ask n-of ratio1 cops [set v 4]
  ask n-of ratio2 cops [set v 3]
  ask n-of ratio3 cops [set v 2]
  ;create-cops round (M3 / 100 * initial-cop-density * .01 * count patches) [
    ;move-to one-of patches with [ not any? turtles-here ]
    ;set v 2
    ;set Defected? false
    ;set NotDefected? true
  ...
  ;create-cops round (M4 / 100 * initial-cop-density * .01 * count patches) [
   ; move-to one-of patches with [ not any? turtles-here ]
    ;set v 1
    ;set Defected? false
    ;set NotDefected? true
...
to go
  ask turtles [
    ...
    if breed = cops and NotDefected? [ determine-enforce ]
  ]
  ; Jailed agents get their term reduced at the end of each clock tick
  ask agents [ if jail-term > 0 [ set jail-term jail-term - 1 ] ]
  ; changed cops remain changed
  ;ask cops [ if Notdefected? [ set NotDefected? false ] ]/////// do i need this?
  ; update agent display
  ask agents [ display-agent ]
  ask cops [ display-cop1 ]
  ask cops [ display-cop2 ]
  ask cops [ display-cop3 ]
  ask cops [ display-cop4 ]
...
to determine-enforce
  set Defected? (s.w > def_T)
  if chance >= 0.9559 and Defected? [set NotDefected? false
  set Pdef 1]
  if chance < 0.9559 and Defected? [set NotDefected? true
                                  set Defected? true
   set Pdef 0]
  ifelse NotDefected? [if any? (agents-on neighborhood) with [ active? ] [
    ; arrest suspect
...
to-report s.w
  set finding1 count agents
  set finding2 count agents with [ active? = true ]
  if finding2 = 0 [set finding2 1]
  if finding2 > 1 [set finding2 count agents with [ active? = true ]]
  report ((1 / (finding1 / finding2)) + (0.7 * 0.8))
end

to-report chance
  (ifelse
    v = 0 [set percentage 25]
    v = 1 [set percentage 45]
    v = 2 [set percentage 65]
    [set percentage 85])
  set Pchance_val Ps1 / 100 * percentage
  report 0.1833 + Pchance_val
end
...
679
Defected Cops
count cops with [Defected? and not NotDefected?]
17
1
11
...
677
want to defect
count cops with [NotDefected? and Defected?]
17
1
11
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count cops with [NotDefected? and Defected?]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count cops with [Defected? and not NotDefected?]"

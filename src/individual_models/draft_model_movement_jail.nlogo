...
to arrest-target
  ask target [
    ...
    ask patches in-radius 1.5 [ set time-since-capture time-since-capture + 5 ]
   ]
end
...
to movement-rule
  ;; a1 * area-value - a2 * (cops-in-vision / active-in-vision) + a3 * (number-of-friends-in-radius) - a4 * distance - a5 * jail term
  let cops-in-new-vision count cops-on patches in-radius agent-vision
  set movement-patch max-one-of patches in-radius agent-vision with [not any? cops-here and not any? rebels-here with [ jailed? = false ] ] [ a1 * area-value - a2 * (cops-in-vision / active-in-vision) ]
        if movement-patch != nobody
        [move-to movement-patch]
end
@#$#@#$#@
